...
class FileDisk extends Model
{
    ...
    public static function validateCredentials($credentials, $disk)
    {
        ...
        try {
            ...
            \Storage::disk($prefix . $disk)->put($root.'dinvoice_temp.text', 'Check Credentials');

            if(\Storage::disk($prefix . $disk)->exists($root.'dinvoice_temp.text')) {
                $exists = true;
                \Storage::disk($prefix . $disk)->delete($root.'dinvoice_temp.text');
            }
        } catch(\Exception $e) {
            $exists = false;
        }
